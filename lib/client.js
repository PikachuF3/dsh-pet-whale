...
`;
  ...
  `;
			...
			const swimmer = new WhaleSwimmer({
				...
				showDialog,
				getStrings: () => strings,
				isBusy: () => root.classList.contains(HIDDEN_CLASS) || dragging || document.hidden || menu.classList.contains("open") || sleeping || visualState !== "idle"
			});
			...
			const onVisibility = () => {
				...
				if (document.hidden) {
					hideTicker();
					swimmer.stop();
				} else if (visualState === "idle") swimmer.scheduleNext(2e3);
			};
